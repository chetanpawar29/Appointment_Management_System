...
<body class="cbp-spmenu-push">
	<div class="main-content">
		...
		try
		{
			...
			{%>
				<tr>
					...
					<td><a href="a_4_deleteEmployee.jsp?empId=<%=rs.getInt(1)%>"><button class="btn btn-default" style="background-color: red">Delete</button></a></td>
				</tr>
			<% }
		}
		catch(Exception e)
        ...
        try {
            ...
            if (accno > 0) {
                Connection con = DBConnection.dbCon();
                PreparedStatement ps = con.prepareStatement("select * from users where accno=?");
                ps.setLong(1, accno);
                ResultSet rs = ps.executeQuery();
                if (rs.next()) {
        %>
        ...
        <tr>
            ...
            <td>RS. <%= rs.getLong(4) %></td>
        </tr>
        <%
                } else {
                    out.println("<tr><td colspan='2' class='error'>Account not found.</td></tr>");
                }
            }
        } catch (Exception e) {
            out.println("<tr><td colspan='2' class='error'>An error occurred while processing your request.</td></tr>");
        }
        }
        %>
    </table>
